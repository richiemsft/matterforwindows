...
public:
    ...
    /**
     * @brief Delete every stored value whose key starts with @p prefix.
     */
    CHIP_ERROR ClearPrefix(const char * prefix);

    /**
     * @brief Delete every stored value except keys starting with @p preservedPrefix.
     */
    CHIP_ERROR ClearAllExceptPrefix(const char * preservedPrefix);
