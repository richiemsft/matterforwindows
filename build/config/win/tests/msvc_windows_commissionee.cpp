...
constexpr char kStorageRoot[]         = "windows-commissionee-kvs";

class CommissioneeDeviceInfoProvider final : public DeviceInstanceInfoProvider
{
public:
    ...
    {
        return CopyString(buffer, bufferSize, "Windows BLE Commissionee");
    }
    ...
    CHIP_ERROR GetPartNumber(char * buffer, size_t bufferSize) override { return CopyString(buffer, bufferSize, "WIN-BLE-1"); }
    ...
    CHIP_ERROR GetSerialNumber(char * buffer, size_t bufferSize) override { return CopyString(buffer, bufferSize, "WINBLE0001"); }
    ...
    CHIP_ERROR GetRotatingDeviceIdUniqueId(MutableByteSpan & uniqueIdSpan) override
    {
        static constexpr uint8_t kUniqueId[16] = { 0x57, 0x49, 0x4E, 0x44, 0x4F, 0x57, 0x53, 0x2D,
                                                   0x42, 0x4C, 0x45, 0x2D, 0x30, 0x30, 0x30, 0x31 };
...
int RunCommissionee(uint32_t runSeconds)
{
    static TestOnlyCommissionableDataProvider commissionableDataProvider;
    static CommissioneeDeviceInfoProvider deviceInfoProvider;
    ...
    if (error == CHIP_NO_ERROR)
    {
        std::printf("Windows Matter commissionee is advertising for %u seconds.\n", runSeconds);
...
int main(int argc, char * argv[])
{
    ...
    const int result = RunCommissionee(runSeconds);
